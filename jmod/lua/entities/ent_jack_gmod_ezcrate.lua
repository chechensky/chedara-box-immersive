...
if SERVER then
	...
	matsda = {
		"gigaconvertedmats/building_template005a",
		"gigaconvertedmats/courtyard_template001a",
		"gigaconvertedmats/courtyard_template003b",
		"gigaconvertedmats/metalshutters001a"
	}
	

	function ENT:Initialize()
		self.Entity:SetModel("models/kali/props/cases/hard case c.mdl")
		self.Entity:SetMaterial(table.Random(matsda))
		...
		self.Entity:SetAngles(Angle(0, (ply:GetPos() - tr.HitPos):Angle().y, 0))
...
elseif CLIENT then
	...
	function ENT:Draw()
		...
		if DetailDraw then
			...
			cam.Start3D2D(Pos + Up * 16 - Forward * 15 + Right, Ang, .10)
			draw.SimpleText("Homice-Box corp.", "JMod-Stencil-S", 0, 0, TxtCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
			...
			cam.Start3D2D(Pos + Up * 16 + Forward * 15 - Right, Ang, .15)
			draw.SimpleText("Homice-Box corp.", "JMod-Stencil-S", 0, 0, TxtCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
			draw.SimpleText(ResourceName, "JMod-Stencil", 0, 15, TxtCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
			draw.SimpleText(Resource .. " UNITS", "JMod-Stencil-S", 0, 70, TxtCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
			cam.End3D2D()
