...
if(SERVER)then
	...
	function ENT:Connect(ply)
		...
		timer.Simple(1, function()
			if IsValid(self) then
				self:Speak("Установлена линия связи с Si.Corp. Радио Форпост " .. OutpostID)
			end
		end)
	end
		...
		if self.NextRealThink < Time then
			...
			if State == STATE_CONNECTING then
				...
				if self:TryFindSky() then
					self:Speak("Трансляция получена, установлена линия связи...")
					...
					if self.ConnectionAttempts > 5 then
						self:Speak("Невозможно установить соединение ни с одной заставой. Выключение.")
			...
			elseif State > 0 then
				...
				if not self:TryFindSky() then
					...
					if self.ConnectionlessThinks > 5 then
						self:Speak("Соединение с заставой потеряно. Выключение.")
	...
	function ENT:EZreceiveSpeech(ply, txt)
		...
		local NormalReq, BFFreq = string.sub(txt, 1, 14) == "supply radio: ", string.sub(txt, 1, 6) == "ahlo: "
		...
		if NormalReq or BFFreq then
			...
			if Name == "help" then
				if State == 2 then
					...
					local Msg, Num = 'встаньте возле рации и скажите в чате "supply radio: status", or "supply radio: [package]". доступные пакеты:', 6
...
elseif(CLIENT)then
	...
	local GlowSprite, StateMsgs = Material("sprites/mat_jack_basicglow"), {
		...
		[JMod.EZ_STATION_STATE_DELIVERING] = "В процессе доставки",
		[JMod.EZ_STATION_STATE_BUSY] = "Линия занята"
	}
	...
	function ENT:Draw()
		...
		if DetailDraw then
			...
			if (Closeness < 20000) and (State > 0) then
				...
				if State > 1 then
					draw.SimpleTextOutlined("Подключено:", "JMod-Display", 0, 0, Color(255, 255, 255, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, Color(0, 0, 0, Opacity))
					draw.SimpleTextOutlined("Si.Corp. Радио Форпост № " .. self:GetOutpostID(), "JMod-Display", 0, 40, Color(255, 255, 255, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, Color(0, 0, 0, Opacity))
				end

				local ElecFrac = self:GetElectricity() / self.MaxElectricity
				local R, G, B = JMod.GoodBadColor(ElecFrac)
				draw.SimpleTextOutlined("Заряд: " .. math.Round(ElecFrac * 100) .. "%", "JMod-Display", 0, 70, Color(R, G, B, Opacity), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, Color(0, 0, 0, Opacity))
				...
				if State == JMod.EZ_STATION_STATE_READY then
					draw.SimpleTextOutlined('ALT + E', "JMod-Display-S", 0, 140, Color(255, 255, 255, Opacity / 2), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, Opacity / 2))
				end
