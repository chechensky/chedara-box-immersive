	...
	if SERVER then
		...
		if SelectedBuild and SelectedBuild ~= "" then
			...
			if not(self:GetElectricity() >= 6 * (buildInfo.sizeScale or 1)) or not(self:GetGas() >= 8 * (buildInfo.sizeScale or 1)) then
				self:Msg("   Вам необходимо пополнить запас бензина и/или энергии.\nНажмите Walk + Use на газе или батареях, чтобы пополнить запасы.")
				return
			end
			...
			if JMod.HaveResourcesToPerformTask(nil, nil, Reqs, self) then
				...
				if override == false then
					self:Msg(msg or "Стройка не доступна")
				...
				if IsValid(Ent) then
					...
					elseif IsValid(Phys) then
						if Alt then
							...
							if SERVER then
								if constraint.HasConstraints(Ent) or not Phys:IsMotionEnabled() then
									self:Msg("Объект приварен или заморожен")
								else
									local Mass = Phys:GetMass() ^ .8
									...
									if #table.GetKeys(Yield) <= 0 then
										...
										JMod.Hint(self.Owner, "work spread")
										local WorkSpreadMult = JMod.CalcWorkSpreadMult(Ent, Pos)
										local AddAmt = 250 / Mass * WorkSpreadMult * 5
										SetAmt = math.Clamp(Prog + AddAmt, 0, 100)
										self:Pawnch()
										sound.Play("snds_jack_gmod/ez_tools/hit.wav", Pos + VectorRand(), 60, math.random(50, 70))
										sound.Play("snds_jack_gmod/ez_dismantling/" .. math.random(1, 10) .. ".wav", Pos, 65, math.random(90, 110))
...
function SWEP:DrawHUD()
	...
	draw.SimpleTextOutlined("Энергия: "..math.floor(self:GetElectricity()), "Trebuchet24", W * .1, H * .5, Color(255, 255, 255, 100), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 50))
	draw.SimpleTextOutlined("Газ: "..math.floor(self:GetGas()), "Trebuchet24", W * .1, H * .5 + 30, Color(255, 255, 255, 100), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 50))
	draw.SimpleTextOutlined("Базовый крепёж: "..math.floor(self:GetBasicParts()), "Trebuchet24", W * .1, H * .5 + 60, Color(255, 255, 255, 100), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 50))

	draw.SimpleTextOutlined("R: Меню крафта", "Trebuchet24", W * .4, H * .7, Color(255, 255, 255, 30), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 10))
	draw.SimpleTextOutlined("ЛКМ: построить или улучшить", "Trebuchet24", W * .4, H * .7 + 30, Color(255, 255, 255, 30), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 10))
	draw.SimpleTextOutlined("ALT+ЛКМ: изменить", "Trebuchet24", W * .4, H * .7 + 60, Color(255, 255, 255, 30), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 10))
	draw.SimpleTextOutlined("ПКМ: Разобрать", "Trebuchet24", W * .4, H * .7 + 90, Color(255, 255, 255, 30), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 10))
	draw.SimpleTextOutlined("ALT+ПКМ: ослабить", "Trebuchet24", W * .4, H * .7 + 120, Color(255, 255, 255, 30), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 10))
	draw.SimpleTextOutlined("Backspace: сброс комплекта", "Trebuchet24", W * .4, H * .7 + 150, Color(255, 255, 255, 30), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 10))
	
	local Prog = self:GetTaskProgress()

	if Prog > 0 then
		draw.SimpleTextOutlined((self.Owner:KeyDown(JMod.Config.AltFunctionKey) and "Ослабление...") or "Разборка...", "Trebuchet24", W * .5, H * .45, Color(255, 255, 255, 100), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 3, Color(0, 0, 0, 50))
		draw.RoundedBox(10, W * .3, H * .5, W * .4, H * .05, Color(0, 0, 0, 100))
		draw.RoundedBox(10, W * .3 + 5, H * .5 + 5, W * .4 * LastProg / 100 - 10, H * .05 - 10, Color(W*255/100, 255, W*255/100, 100))
	end
