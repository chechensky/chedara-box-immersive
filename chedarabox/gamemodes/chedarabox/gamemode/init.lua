...
function GM:PlayerDeathThink(ply)
	...
	if key ~= ply.oldKeyWalk and key then
		...
		ply:ChatPrint(ply.EnableSpectate and "Spectating players" or "Noclip")
	end

	ply.oldKeyWalk = key

	ply.SpectateGuy = ply.SpectateGuy or 1
	...
	if ply.EnableSpectate then
		...
		if ply.oldKeyAttack1 ~= key1 and key1 then
			...
			if ply.SpectateGuy > #tbl then ply.SpectateGuy = 0 end
		...
		if not IsValid(spec) then ply.SpectateGuy = 0 return end
		ply:SetPos(spec:GetPos() + Vector(5,0,50))
		local spec = spec
		...
		if ply:KeyDown(IN_ATTACK) then
			local tr = {}
			tr.start = ply:GetPos()
			tr.endpos = tr.start + ply:GetAimVector() * 6
			tr.filter = ply
