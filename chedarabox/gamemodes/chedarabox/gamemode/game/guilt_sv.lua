	...
	if att.Guilt >= 100 then
		...
		if not att.noguilt and not att:HasGodMode() then
			...
			if not validUserGroup[att:GetUserGroup()] then
				RunConsoleCommand("ulx","fakeban",att:Name(),"10","рдмер хули")
			else
				RunConsoleCommand("ulx","fakeban",att:Name(),"10","рдмер хули")
			end
		end
...
hook.Add("PlayerInitialSpawn","guiltasdd",function(ply)
	...
	ply:ChatPrint("Your guilt: " .. tostring(ply.Guilt) .. "%")
	ply.RoundGuilt = 0
end)
...
hook.Add("PlayerSpawn","guilt",function(ply)
    ...
    ply.have_pso1m2 = false
    ply:SetNWBool("PLY_PSO1M2", false)
    ply.have_leupoldmark4 = false
    ply:SetNWBool("PLY_Mark4Leupold", false)
    ply.have_okp7sight = false
    ply:SetNWBool("PLY_OKPSight7", false)
    ply.have_eotech553 = false
    ply:SetNWBool("PLY_EOTech553", false)
    ply.have_aksuppressor = false
    ply:SetNWBool("PLY_Supressor762", false)
    ply.have_12suppressor = false
    ply:SetNWBool("PLY_Supressor12Gauge", false)
    ply.have_9x19suppressor = false
    ply:SetNWBool("PLY_Supressor9x19", false)
	ply:ChatPrint("Your guilt: " .. tostring(math.floor(ply.Guilt)) .. "%")
