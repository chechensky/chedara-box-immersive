...
local function ToggleScoreboard(toggle)
	if toggle then
		...
		HomigradScoreboard:SetSize(scrw*.4,scrh*.6)
		HomigradScoreboard:SetPos(30, 50)
		...
		HomigradScoreboard.Paint = function(self,w,h)
			...
			surface.SetDrawColor(14,14,14,55)
			surface.DrawRect(0,0,w,h)

			draw.SimpleText("Status","HomigradFontBig",100,15,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			draw.SimpleText("Name","HomigradFontBig",w / 3,15,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)

			draw.SimpleText("CHEDARA BOX","HomigradFontLarge",w / 2,h / 2,Color(155,155,165,5),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			
			draw.SimpleText("Played","HomigradFontBig",w - 350,15,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			...
			draw.SimpleText("Ping","HomigradFontBig",w - 250,15,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			draw.SimpleText("Team","HomigradFontBig",w - 150,15,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			...
			draw.SimpleText("TPS Server: " .. tick,"HomigradFont",w - 755,h - 45,tick <= 35 and red or green,TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
		...
		function HomigradScoreboard:AddPlayer(ply)
			...
			playerPanel.DoClick = function()
				...
				playerMenu:AddOption("OpenProfile", function()
					ply:ShowProfile()
				end)
			...
			playerPanel.Paint = function(self,w,h)
				...
				draw.SimpleText(name1,"HomigradFont",w / 3,h / 2,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
				...
				if not ply.TimeStart then
					...
					draw.SimpleText(dTime,"HomigradFont",w - 350 - 15,h / 2,white,TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER)
					draw.SimpleText(hTime,"HomigradFont",w - 350,h / 2,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
					draw.SimpleText(mTime,"HomigradFont",w - 350 + 15,h / 2,white,TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
				end
				
				draw.SimpleText(ply:Ping(),"HomigradFont",w - 250,h / 2,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
				...
				draw.SimpleText(name,"HomigradFont",w - 150,h / 2,color,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			end
			...
			if ply ~= LocalPlayer() then
				...
				button:SetPos(playerPanel:GetWide() - playerPanel:GetTall() * 1.8 - 32 / 2,h)
