...
local count = math.min(math.floor(#players / 1), 1)
for i = 1, count do
    local ply, key
    repeat
        ply, key = table.Random(players)
    until not string.find(ply:Nick(), "Bot")
    players[key] = nil
		...
		for i,ply in pairs(team.GetPlayers(2)) do
			...
			for i,point in pairs(list) do
				if ply:GetPos():Distance(point[1]) < (point[3] or 250) then
					...
					PrintMessage(3,"Islamists " .. (CTAlive - 1) .. " школьников")
				end
			end
		end
	end
...
local rare = {"weapon_glock18","weapon_gurkha","weapon_t"}
