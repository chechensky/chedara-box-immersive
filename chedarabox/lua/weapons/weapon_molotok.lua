...
function SWEP:SecondaryAttack()
    if not self.mode then
        ...
        if self:GetOwner():GetAmmoCount("nails") == 0 then return end
        if SERVER then
            self:GetOwner():SetAmmo(self:GetOwner():GetAmmoCount("nails") - 1,"nails")
...
function SWEP:DrawHUD()
    ...
    draw.DrawText("Гвозди: "..self:GetOwner():GetAmmoCount("nails"),"TargetID",traceResult.HitPos:ToScreen().x,traceResult.HitPos:ToScreen().y + 20,color_white,TEXT_ALIGN_CENTER)
