...
hook.Add("HomigradDamage","PlayerPainGrowth",function(ply,hitGroup,dmginfo,rag,armorMul)
	...
	elseif not dmginfo:IsDamageType(DMG_BLAST+DMG_NERVEGAS) then
		...
		if ply.painlosing > 10 or ply.pain > 600 + ply:GetNWInt("SharpenAMT") * 5 or ply.Blood < 3000 and not ply.Otrub then
			ply.gotuncon = true
		end
	end
...
hook.Add("Player Think","homigrad-pain",function(ply,time)
	...
	if ply.painlosing > 5 then
		ply.stamina = 30
		ply.pain = ply.pain + 2 * ply.painlosing
		--ply.KillReason = "painlosing"
		--ply:Kill()
	end

	if ply.pain >= 1800 then
		ply.KillReason = (ply.painlosing > 5 and "painlosing") or "pain"
		ply:Kill()
		return
	end
...
function IsUnconscious(ply)
	if ply.painlosing > 20 or ply.pain > 1000 + ply:GetNWInt("SharpenAMT") * 5 or ply.Blood < 3000 or ply.heartstop or ply.o2 < 0.45 then
		ply.Otrub = true
		ply:SetDSP(16)
