...
hook.Add("EntityTakeDamage","ragdamage",function(ent,dmginfo) --урон по разным костям регдолла
	...
	for armorInfo,armorData in pairs(armors) do
		...
		if dmginfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT) then
			...
			elseif
				...
				slots.leftcalf or
				slots.rightcalf or
				slots.tourniquetlefthand or
				slots.tourniquetrighthand or 
				slots.tourniquetleftleg or
				slots.tourniquetrightleg
			then
	...
	dmginfo:SetDamage( haveHelmet and (dmginfo:GetDamage() * armorMul) * 0.2 or (dmginfo:GetDamage() * armorMul) * 0.6)
