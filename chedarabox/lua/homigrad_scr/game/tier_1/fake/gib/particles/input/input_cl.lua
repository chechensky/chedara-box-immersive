local mats = {}
for i = 1,2 do mats[i] = Material("decals/checha/bloodparticle" .. i) end
...
local function addBloodPartArt(pos,vel,mat,w,h,ply)
	pos = pos + vecZero
	vel = vel + vecZero
	local pos2 = Vector()
	pos2:Set(pos)
	
	bloodparticels1[#bloodparticels1 + 1] = {pos,pos2,vel,mat,w,h,ply}
end
net.Receive("blood particle",function()
	local pos,vel = net.ReadVector(),net.ReadVector()
	addBloodPart(pos + Vector(0,0,0), vel, mats[random(1,#mats)], 3,3)
end)
...
net.Receive("blood particle arteryWHO",function()
	local pos,vel,ply = net.ReadVector(),net.ReadVector(),net.ReadEntity()
	addBloodPartArt(pos + Vector(math.Rand(-1.7,1.2),math.Rand(-2,3),math.Rand(-2,6)), vel, mats[random(1,#mats)], 2, 2.5, ply)
end)
