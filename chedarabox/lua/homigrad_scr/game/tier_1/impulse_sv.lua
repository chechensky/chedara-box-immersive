...
hook.Add("HomigradDamage","ImpulseShock",function(ply,hitGroup,dmginfo)
	...
	if hitGroup == HITGROUP_RIGHTLEG or hitGroup == HITGROUP_LEFTLEG then
		if ply.dmgimpulse > 7 then timer.Simple(0,function() if not ply.fake then Faking(ply) end end) end
	end
	...
	if hitGroup == HITGROUP_STOMACH then
		if ply.dmgimpulse > 18 then timer.Simple(0,function() if not ply.fake then Faking(ply) end end) end
	end
end)
